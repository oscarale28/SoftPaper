...
if (isset($_GET['uuid_producto'])){
    ...
    if ($producto->setId($_GET['uuid_producto'])) {
        ...
        if ($rowproducto = $producto->readOne()) {
            ...
            $pdf->startReport('Flujo de stock');
                ...
                if ($productoInfo = $producto->flujoStockReportInfo()){
                ...
                if ($dataproducto = $producto->flujoStockReport($_GET['start'], $_GET['end'])) {
                    ...
                    $pdf->cell(62, 10, utf8_decode('Color'), 0, 0, 'C', 1);//Imprime una celda (área rectangular) con bordes opcionales, color de fondo y cadena de caracteres-->
                    $pdf->cell(62, 10, utf8_decode('Stock ingresado'), 0, 0, 'C', 1);//Imprime una celda (área rectangular) con bordes opcionales, color de fondo y cadena de caracteres-->
                    $pdf->cell(62, 10, utf8_decode('Fecha modificación'), 0, 1, 'C', 1);
                    ...
                    foreach ($dataproducto as $rowproducto) {
                        ...
                        $pdf->cell(62, 10, $rowproducto['color_producto'], 'B', 0,'C');
                        $pdf->cell(62, 10, $rowproducto['stock_agregado'], 'B', 0,'C');
                        $pdf->cell(62, 10, $rowproducto['fecha_bitacora'], 'B', 1,'C');
                    }
                } else {
                    $pdf->cell(0, 10, utf8_decode('No flujo para mostrar'), 1, 1);
                }
                // Se envía el documento al navegador y se llama al método footer()
                $pdf->output('I', 'flujo_stock.pdf');
