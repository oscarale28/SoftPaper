...
if ($venta->setId($_GET['uuid_venta'])) {
    if ($infoClie = $venta->detalleClie()) {
        ...
        $pdf->cell(45, 5, utf8_decode($infoClie['nombre_cliente']), 0, 0, 'R');
        $pdf->cell(20, 5, "", 0, 0, 'R');
        ...
        $pdf->setFont('Mohave-Light','',8);
        $pdf->cell(50, 5, utf8_decode($infoClie['nit_cliente']), 0, 0, 'R');
        $pdf->cell(15, 5, "", 0, 0, 'R');
        $pdf->cell(35, 5, utf8_decode($infoClie['telefono_cliente']), 0, 0, 'R');
        $pdf->ln(4);

        //celda 3 Direccion 
        $pdf->setFont('Mohave-Light','',8);
        $pdf->cell(50, 5, utf8_decode($infoClie['direccion_cliente']), 0, 0, 'R');
        $pdf->cell(10, 5, "", 0, 0, 'R');
        $pdf->cell(35, 5, "", 0, 0, 'R');
        $pdf->ln(4);
        ...
        if ($monto = $venta->montoConsumidorFinal()){
            ...
            $pdf->ln(55);
            //Monto de la venta sin el iva-------------------------
            $pdf->setFont('Mohave-Light','',8);
            $pdf->cell(100, 5, "$".number_format($monto['monto_total'] ), 0, 1, 'R');
            $pdf->ln(6);

            //Monto de la venta con el iva------------------------------
...
$pdf->output('I', 'consumidor_final.pdf');