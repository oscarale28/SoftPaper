    ...
    if (isset($_SESSION['uuid_empleado'])) {
        ...
        switch ($_GET['action']) {
            ...
            case 'delete':
                ...
                break;
            case 'change':
                if (!$estadoVenta->setId($_POST['id_delete1'])) {
                    $result['exception'] = 'Estado de venta incorrecto';
                } elseif (!$estadoVenta->setNum($_POST['num'])) {
                    $result['exception'] = 'Estado de venta incorrecto';
                } elseif (!$estadoVenta->readOne()) {
                    $result['exception'] = 'Estado de venta inexistente';
                } elseif ($estadoVenta->changeRow()) {
                    $result['status'] = 1;
