...
require_once('../models/estado_empleado.php');
...
if (isset($_GET['action'])) {
    ...
    if (isset($_SESSION['uuid_empleado'])) {
        ...
        switch ($_GET['action']) {
            ...
            case 'readAll':
                if ($result['dataset'] = $estadoEmpleado->readAll()) {
                    $result['status'] = 1;
            ...
            case 'readOne':
                if (!$estadoEmpleado->setId($_POST['id'])) {
                    $result['exception'] = 'Estado de producto incorrecto';
                } elseif ($result['dataset'] = $estadoEmpleado->readOne()) {
                    $result['status'] = 1;
                } elseif (Database::getException()) {
                    $result['exception'] = Database::getException();
                } else {
