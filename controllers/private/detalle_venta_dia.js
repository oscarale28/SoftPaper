const API_VENTAS = SERVER + "private/venta_dia.php?action=";
        ...
        if (request.ok) {
            ...
            request.json().then(function (response) {
                ...
                if (response.status) {
                    ...
                    document.getElementById("iva").textContent = `$${parseFloat(
                        iva.toFixed(2)
                    )}`;
                    document.getElementById("total-venta").textContent = `$${parseFloat(
                        totalVenta.toFixed(2)
                    )}`;
                    document.getElementById("monto").value = parseFloat(
                        totalVenta.toFixed(2)
                    );
                } else {
                    document.getElementById(
...
//Función para contar los productos de la venta y mostrar contador
...
function finishOrder() {
    if (document.getElementById("correlativo").value == "") {
        ...
        swal({
            title: "Aviso",
            text: "¿Está seguro de finalizar la factura del día?",
            icon: "info",
            buttons: ["No", "Sí"],
            ...
            if (value) {
                ...
                data.append("idVenta", idVenta);
                data.append("correlativo", document.getElementById("correlativo").value);
                console.log(idVenta);
                                                        ...
                                                        // Se verifica si la petición es correcta, de lo contrario se muestra un mensaje en la consola indicando el problema.
                                                            ...
                                                            // Se obtiene la respuesta en formato JSON.
                                                                ...
                                                                if (response.status) {
                                                                    ...
                                                                    url = SERVER + 'reports/venta_dias.php';
